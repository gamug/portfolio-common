...
from portfolio_common.news_nlp import schema
...
def test_init_schema_creates_every_result_table(news_nlp_db_path: Path) -> None:
    conn = sqlite3.connect(news_nlp_db_path)
    names = {r[0] for r in conn.execute("SELECT name FROM sqlite_master WHERE type='table'")}
    conn.close()
    assert names >= _RESULT_TABLES


def test_init_schema_is_idempotent(news_nlp_db_path: Path) -> None:
    conn = db.connect(news_nlp_db_path)
