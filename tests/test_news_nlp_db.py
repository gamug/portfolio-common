...
a lean ``articles`` subset). See docs/news-nlp-db-topology.md.
"""
...
import pytest
from conftest import NEWS_NLP_LEAN_ARTICLES_SCHEMA
...
def test_require_source_text_raises_when_source_lacks_body_text(
    ...
    setup.executescript(NEWS_NLP_LEAN_ARTICLES_SCHEMA)
    setup.commit()
    setup.close()
