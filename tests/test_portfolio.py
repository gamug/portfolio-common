"""Tests for portfolio_common/portfolio.py -- the live S&P 500 universe scrape and
its as_of delegation to portfolio_common.universe_history. No test hits the network:
...
from portfolio_common import portfolio
...
def test_list_universe_default_never_touches_universe_history(
    ...
    of portfolio_common.universe_history at all."""
    monkeypatch.setattr(portfolio, "load_universe", _fake_universe_df)
    poison = MagicMock(side_effect=AssertionError("must not be called"))
    monkeypatch.setattr("portfolio_common.universe_history.query_as_of", poison)
...
def test_list_universe_with_as_of_delegates_to_universe_history(
    ...
    monkeypatch.setattr("portfolio_common.universe_history.query_as_of", mock_query)
...
def test_resolve_symbol_with_as_of_delegates_to_universe_history(
    ...
    monkeypatch.setattr("portfolio_common.universe_history.resolve_as_of", mock_resolve)
