...
Query layer (the only SQL in this module):
    ``_distinct_ids``      one ``SELECT DISTINCT`` per required-data table, tolerant
                            of a table that doesn't exist yet in a partial DB.
    ``check_coverage``     reads ``score_snapshot`` / ``fundamental_metrics`` /
                            ``price_daily`` / ``quant_return_daily`` / ``price_observation``
                            plus (via :mod:`.universe_source`) ``universe.db``.
    ``persist_coverage``   upserts one ``universe_coverage`` row per member.

``SymbolCoverage`` / ``CoverageReport`` (dataclasses below the query layer) are
pure business logic -- no SQL, just the per-member/report-level roll-up
properties (``covered``, ``missing``, ``fraction``, ...).
"""
...
from .universe_source import resolve_asset_ids, symbols_asof
