...
Query layer (every function here is a pure read, no writes):
    ``members_asof``      the open membership stints as of a date, joined out to
                           full ``UniverseMember`` rows.
    ``symbols_asof``      just the ticker symbols from ``members_asof``.
    ``resolve_asset_ids``  case-insensitive ticker -> ``assets.id`` lookup against
                           the financial DB (chunked ``IN (...)``, never inserts).
"""
...
def connect_ro(path: str | Path) -> Database:
    ...
    here so callers keep importing ``connect_ro`` from
    ``kg_schema.universe_source``."""
