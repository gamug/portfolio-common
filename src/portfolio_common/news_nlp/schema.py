...
Does **not** create ``articles`` -- that table is owned by the crawler
(``portfolio_common.schema``) on the SOURCE side; on the RESULTS side a lean,
``body_text``-free subset is populated row-by-row by
``portfolio_common.news_nlp.db._ensure_article_row``.
"""
