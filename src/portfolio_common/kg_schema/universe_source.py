...
def connect_ro(path: str | Path) -> sqlite3.Connection:
    ...
    The shared read-only factory (:func:`portfolio_common.kg_schema.connect_ro`), re-exposed here
    so callers keep importing ``connect_ro`` from ``portfolio_common.kg_schema.universe_source``."""
