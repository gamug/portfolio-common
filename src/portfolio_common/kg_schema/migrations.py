...
These NEVER run from :func:`portfolio_common.kg_schema.ensure`'s automatic path. They run only via
...
def _m002_financial_facts(conn: sqlite3.Connection) -> None:
    ...
    if "filing_version" not in cols or "event_time" not in cols:
        return  # additive columns not present yet; run portfolio_common.kg_schema.ensure first
