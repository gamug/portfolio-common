...
``ensure(conn, run_migrations=True)``  additionally runs the non-additive rebuilds in
                                 :mod:`portfolio_common.kg_schema.migrations`. Only ``python -m <agent>
                                 migrate`` passes this.
"""
...
from portfolio_common.kg_schema import version as _version
from portfolio_common.kg_schema.db import connect, connect_ro
from portfolio_common.kg_schema.ddl import ADDITIVE_DDL, REQUIRED_COLUMNS
from portfolio_common.kg_schema.env import (
    DB_ENV_VAR,
    LEGACY_DB_ENV_VAR,
    UNIVERSE_DB_ENV_VAR,
    database_path,
...
from portfolio_common.kg_schema.migrations import apply_migrations
from portfolio_common.kg_schema.provenance import code_version
from portfolio_common.kg_schema.views import ensure_views
